import { Invoice as InvoiceType, Shipment, Vehicle, Driver } from '@/lib/types';
...
export function Invoice({ invoice, shipment, vehicle, driver }: InvoiceProps) {
    ...
    <div className="bg-white text-black p-8 rounded-lg shadow-lg max-w-4xl mx-auto my-8 print-container">
      <header className="flex justify-between items-start pb-4 border-b-2 border-gray-800">
        ...
        <div className="text-right">
          <h2 className="text-2xl font-bold uppercase">Tax Invoice</h2>
      ...
      <section className="grid grid-cols-2 mt-4">
        <div>
          ...
          <p className="text-sm">{numberToWords(invoice.total)}</p>
        </div>
        <div className="text-right">
          <div className="grid grid-cols-2 gap-x-4">
            <p>Subtotal:</p><p>₹{invoice.subtotal.toFixed(2)}</p>
            {invoice.cgst > 0 && <>
              <p>CGST (9%):</p><p>₹{invoice.cgst.toFixed(2)}</p>
              <p>SGST (9%):</p><p>₹{invoice.sgst.toFixed(2)}</p>
            </>}
            {invoice.igst > 0 && <>
              <p>IGST (18%):</p><p>₹{invoice.igst.toFixed(2)}</p>
            </>}
            <p className="font-bold border-t-2 border-gray-800 pt-1 mt-1">Total:</p>
            <p className="font-bold border-t-2 border-gray-800 pt-1 mt-1">₹{invoice.total.toFixed(2)}</p>
          </div>
        </div>
      </section>
